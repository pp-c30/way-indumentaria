...
export class productoController {
    ...
    public async listaProducto(req:Request,res:Response){
       ...
       const db = await conexion();
        ...
        let producto = await db.query('select p.id_producto, p.codigo, p.descripcion, p.precio_compra, p.precio_way, p.precio_final, c.descripcion as descripcion_categoria, p.estado, p.descuento, p.categoria_sexo, p.fecha_carga from producto p, categoria c where p.categoria = c.id_categoria');
