    ...
    listaVentas(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                let ventas = yield db.query('select concat("v_",vr.id_vendedor) as id_vendedor_busqueda,DATE_FORMAT(v.fecha_venta,"%d/%m/%Y") as fecha_venta,DATE_FORMAT(v.fecha_venta,"%Y-%m-%d") as fecha_venta_origin, DATE_FORMAT(v.fecha_venta,"%d/%m/%Y") as fecha_venta_formateada, DATE_FORMAT(v.fecha_venta, "%d") as day, DATE_FORMAT(v.fecha_venta, "%m") as month, DATE_FORMAT(v.fecha_venta, "%Y") as year, v.id_venta, p.descripcion as producto_descripcion, p.id_producto as producto, p.codigo as codigo_producto, v.cantidad, v.importe, v.fecha_venta, v.importe_unitario, v.estado, v.forma_pago, v.descuento_aplicado, v.vendedor as vendedor_venta, vr.nombre from venta v, producto p,vendedor vr where v.producto = p.id_producto and v.vendedor = vr.id_vendedor');
                yield db.end();
                res.json(ventas);
            }
            catch (error) {
            ...
            try {
                ...
                if (producto[0]) {
                    if (producto[0].stock > 0) {
                        ...
                        yield db.query('update producto set stock = stock-1 where codigo = ?', [req.body.codigo_producto]);
